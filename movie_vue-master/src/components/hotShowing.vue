      ...
      <el-row v-else class="flex">
        <el-col :span="6" v-for="item in movies" :key="item.movieId" style="padding: 30px">
          ...
          <div style="text-align: center" class="movie_name">
            ...
            <span style="font-size: 30px;color: #fcac18;display: block;font-style: italic" v-else>
  ...
  methods: {
    ...
    search(){
      ...
      this.$axios({
        ...
        params: {type: searchTypes, area: searchArea,
          year: searchYear, showType: 1,order: this.checked}
      }).then(res => {
    ...
    toOrder(checked){
      ...
      this.$axios({
        url: '/movie/moviesByChoose',
        params: {type: searchTypes, area: searchArea,
          year: searchYear,showType: 1,order: checked}
      }).then(res => {
        this.movies = res.data.data
        this.loading = false
      })

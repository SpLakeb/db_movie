    ...
    <div class="main" style="width: 1000px;margin: 180px auto 0 auto">
      <div style="width: 100%">
        ...
        <p class="board-content">
          榜单规则：将国内热映的影片，按照总票房从高到低排列取前10名，每天上午10点更新。相关数据来源于“天天专业版”及“天天电影库”。
        </p>
      </div>
      <dl style="width: 1194px; margin-top: 50px">
        <dd style="width: 100%">
          <el-row v-for="(item,index) in this.boxTopMovie" :key="index" style="margin: 20px">
            <div>
              ...
              <el-col :span="4" :offset="1" style="background: #fff;height: 232px">
                <el-image :src="item.imgUrl" @click="movieDetails(item.movieId)"
                          style="cursor: pointer;width: 160px;height: 220px"></el-image>
              </el-col>

              <el-col :span="16" :offset="1"
                      style="background: #fff;height: 220px;border-bottom: 1px solid #c9c6c6">
                <div class="board-item-content">
                  ...
                  <div class="movie-item-number wish">
                    <p class="month-wish">总票房:&nbsp;&nbsp;
                      <span>
                        <span class="stonefont"
                              v-if="item.boxOffice > 100000000"> {{ (item.boxOffice / 100000000).toFixed(1) + '亿' }}
                        </span>
                        <span class="stonefont"
                              v-else-if="item.boxOffice > 10000"> {{ (item.boxOffice / 10000).toFixed(1) + '万' }}
                        </span>
                      </span>
                    </p>
                  </div>
...
export default {
  ...
  mounted() {
    this.$axios({
      url: 'rank/getBoxTopMovie',
    }).then(res => this.boxTopMovie = res.data.data)
  }
}
</script>

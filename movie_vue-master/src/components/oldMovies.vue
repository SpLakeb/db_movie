<template>
  ...
  padding: 10px">
    ...
    <div style="width: 1100px;margin: 100px auto 0 auto;text-align: left">
      ...
      <el-checkbox v-model="checked" true-label="true" false-label="false"
                   class="myRedCheckBox"
                   @change="toOrder">
        <span v-if="loading" style="font-size: 20px">重新排序中
          <i class="el-icon-loading"></i>
        </span>
        <span v-else style="font-size: 20px">按评分排序</span>
      </el-checkbox>
...
<!--            <span style="font-size: 20px;color: #fcac18;display: block;font-style: italic"-->
<!--&lt;!&ndash;                  v-if="item.commentsNumber === 0">&ndash;&gt;-->
<!--&lt;!&ndash;              暂无评分&ndash;&gt;-->
<!--            </span>-->
            <span style="font-size: 30px;color: #fcac18;display: block;font-style: italic" >
  ...
  methods: {
    ...
    search(){
      ...
      this.$axios({
        ...
        params: {type: searchTypes, area: searchArea,
          year: searchYear, showType: 3,order: this.checked}
      }).then(res => {
    ...
    toOrder(checked){
      ...
      this.$axios({
        url: '/movie/moviesByChoose',
        params: {type: searchTypes, area: searchArea,
          year: searchYear,showType: 3,order: checked}
      }).then(res => {
        this.movies = res.data.data
        this.loading = false
      })
